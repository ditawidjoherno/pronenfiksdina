    ...
    <div className="max-w-2xl mx-auto p-4 sm:p-6 bg-white shadow-xl rounded-2xl font-[Poppins] border border-gray-200 mt-2">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2 items-center">
          <select
            className="px-2 py-1 sm:px-3 sm:py-2 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
      ...
      {/* Calendar Days */}
      <div className="grid grid-cols-7 text-center gap-1 text-xs sm:text-base">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              key={day}
              className={`p-2 sm:p-4 text-center relative cursor-pointer rounded-lg ${bgColor} hover:bg-gray-300 transition-all`}
              onClick={() => {
                if (!isPast) {
            ...
            >
              ...
              {events[dateKey] && (
                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-500 rounded-full absolute bottom-1 left-1/2 transform -translate-x-1/2"></div>
              )}
            </div>
          );
        })}
      ...
      >
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-xl max-w-sm w-full">
          <h3 className="text-md sm:text-lg font-semibold mb-3">
            Manage Note for {selectedDate}
          </h3>
          <input
            type="text"
            className="w-full p-2 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"
            value={note}
            onChange={(e) => setNote(e.target.value)}
          />
          <div className="flex flex-col sm:flex-row justify-end mt-4 gap-2">
            <button
              className="bg-gray-300 text-black px-4 py-2 rounded-md shadow hover:bg-gray-400 transition-all"
