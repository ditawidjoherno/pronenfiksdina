...
export default function ListForm( ) {
  ...
  const [cost] = useState("Rp 50.000");
    ...
    const [pameranFetched, setPameranFetched] = useState(false);
...
useEffect(() => {
  ...
  const fetchPameran = async () => {
    try {
      ...
      if (!res.ok) throw new Error("Gagal mengambil data  pameran");
...
const handleSave = async () => {
  ...
  const PameranArray = Object.entries(attendance).map(([nisn, value]) => ({
    nisn,
    status: value.status,
    waktu_daftar: value.time, // waktu daftar, contoh "08:30"
    tanggal_daftar: today,     // tanggal daftar diisi hari ini
  }));

  const payload = {
    kelas,
    tanggal_kegiatan: today,     // tanggal kegiatan
    hari_kegiatan: day,          // hari kegiatan, dari formData.date
    batas_pendaftaran: endTime,  // batas pendaftaran, dari formData.end (asumsinya sudah format YYYY-MM-DD)
    biaya: parseInt(formData.cost.replace(/[^\d]/g, '')) || 0, // biaya sebagai number
