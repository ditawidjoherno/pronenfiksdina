...
const ActivityTable = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/api/semua-kegiatan")  // ganti sesuai route API kamu
      .then(res => res.json())
      .then(data => {
        if (data.status === "success") {
          // Map data supaya cocok format frontend
          const mapped = data.data.map(item => ({
            name: item.nama_kegiatan,
            category: item.category,
            start: new Date(item.start).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" }),
            end: new Date(item.end).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" }),
